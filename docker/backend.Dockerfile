...
RUN apt-get update && apt-get install -y --no-install-recommends ffmpeg git libgomp1 && rm -rf /var/lib/apt/lists/*
WORKDIR /app
...
COPY runs ./runs
RUN pip install --no-cache-dir --no-deps . && python -m riskfusion.cli fetch-models
